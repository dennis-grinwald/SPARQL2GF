...
public class SparqlToGfTranslator extends OpVisitorBase {
	...
	// Write SparqlToGfTranslator-builder for object sharing
	...
	public static Dataset<Row> translateQuery(GraphFrame graphframe, String queryString) {
		...
		// GraphFrame query with PROJECTION/SELECTION enabled - allow more than 1
		// variable selection in the future!
		if (motifSelectionList.size() != 0) {
			...
			case 1:
				return gfQuery = graphframe.find(motifPattern).filter(motifFilterList.get(0))
						.select(motifSelectionList.get(0));

			case 2:
				return gfQuery = graphframe.find(motifPattern).filter(motifFilterList.get(0))
						.filter(motifFilterList.get(1)).select(motifSelectionList.get(0));

			case 3:
				return gfQuery = graphframe.find(motifPattern).filter(motifFilterList.get(0))
						.filter(motifFilterList.get(1)).filter(motifFilterList.get(2))
						.select(motifSelectionList.get(0));
